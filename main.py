import streamlit as stfrom datetime import datetimeimport randomimport streamlit.components.v1 as components
...
if "profile" not in st.session_state:st.session_state.profile = {"name": "Abhishek","upi": "abhi@okicici","bank": "ICICI Bank","mask": "XXXX 1234"}

if "popup" not in st.session_state:st.session_state.popup = None

if "qr_result" not in st.session_state:st.session_state.qr_result = ""

---------- QR VALIDATION ----------

def validate_qr(data):if not data:return "invalid"if data.startswith("upi://"):return "upi"if data.startswith("http"):return "suspicious"return "invalid"
